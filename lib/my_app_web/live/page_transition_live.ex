defmodule MyAppWeb.PageTransitionLive do
  ...
  def render(assigns) do
    ...
    <div
      id={"card-#{@index}"}
      class="absolute inset-0 flex bg-orange-100 px-4 py-8 ring-2 ring-orange-500"
      phx-mounted={card_mounted_js()}
      phx-remove={card_remove_js()}
    >
      <div class="mr-8 flex h-full flex-col border-r-4 border-dotted border-orange-200 pr-4">
        <div class="flex flex-1 items-start">
          <div class="size-6 inset-shadow-sm rounded-full border border-orange-200 bg-white" />
        </div>
  ...
  def handle_params(%{"index" => index}, _uri, socket) do
    ...
    {:noreply,
     ...
     |> assign(:prev_path, prev_index && ~p"/example/page-transition?index=#{prev_index}")
     |> assign(:next_path, next_index && ~p"/example/page-transition?index=#{next_index}")}
  end

  def handle_params(_params, _uri, socket) do
    {:noreply, redirect(socket, to: ~p"/example/page-transition?index=0")}
  end
    ...
    JS.transition(
      {
        "transition-all delay-200 duration-300 ease-out " <>
          "group-[.card-prev]/page-transition:z-10 " <>
          "group-[.card-next]/page-transition:-z-10",
        "group-[.card-prev]/page-transition:translate-x-[-100%] " <>
          "group-[.card-next]/page-transition:scale-90",
        "translate-x-[0%] scale-100"
      },
      time: 500
    )
    |> JS.remove_class("card-change card-prev card-next", to: "body", transition: "_", time: 500)
  end
  ...
  def card_remove_js() do
    JS.hide(
      transition: {
        ...
        "translate-x-[0%] scale-100",
        "group-[.card-prev]/page-transition:scale-90 " <>
